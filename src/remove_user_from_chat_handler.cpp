...
RemoveUserFromChatHandler::RemoveUserFromChatHandler(
...
std::string RemoveUserFromChatHandler::HandleRequestThrow(
  ...
  if (requester_id == user_id) {
    response.SetStatus(userver::server::http::HttpStatus::kBadRequest);
    return utils_handler::MakeErrorJson(
        "Use leave endpoint to remove yourself from chat");
  }
  ...
  try {
    ...
    return utils_handler::MakeErrorJson("Failed to remove user from chat");
  }
}
